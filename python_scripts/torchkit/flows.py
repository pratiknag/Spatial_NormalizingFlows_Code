...
#         self.out_to_dsparams.weight.data.normal_(-0.2, 0.2)
#         self.out_to_dsparams.bias.data.normal_(0,0.2)
...
class SigmoidFlow(BaseFlow):
    ...
    def forward(self, x, logdet, dsparams, mollify=0.0, delta=nn_.delta):
        ...
        logj = torchkit.utils.log_sum_exp(logj,2).sum(2)
...
class IAF_DDSF(BaseFlow):
    ...
    def __init__(self, dim, hid_dim, context_dim, num_layers,
        ...
        # Initialize the total number of params for the DenseSigmoidFlow 
        num_dsparams = 0                          
        for i in range(num_ds_layers):                             # num_ds_layers = number of hidden layers of the DDSF model 
            if i == 0:
                in_dim = 1
            else:
                in_dim = num_ds_dim                                # in_dim = input dimension, out_dim = output dimension
            ...
            # num_dsparams = total number of parameters for the DenseSigmoidFlow after ith layer
 ...
 # The parameters of DenseSigmoidFlow are constructed through Conv1d, by taking x as inputs. 
 # This makes the paramters dependent on the data.
    ...
    def reset_parameters(self):                                   # Initializing the params
        self.out_to_dsparams.weight.data.uniform_(-0.001, 0.001)
        self.out_to_dsparams.bias.data.uniform_(0.0, 0.0)
        
            
    def forward(self, inputs):
        x, logdet, context = inputs                               # Inputs are obtained 
        out, _ = self.mdl((x, context))
        out = out.permute(0,2,1)
        
        # parameter set dependent on x has been created and stored inside dsparams
        dsparams = self.out_to_dsparams(out).permute(0,2,1)       
        ...
        lgd = Variable(torch.from_numpy(
            np.zeros((n, dim, 1, 1)).astype('float32')))          # logdet and lgd are not relevant for our purpose.
        ...
        for i in range(self.num_ds_layers):
            ...
            end = start + u_dim + w_dim + a_dim + b_dim          # dimensions for each layer is assigned
            
            params = dsparams[:,:,start:end]
            h, lgd = getattr(self,'sf{}'.format(i))(h, lgd, params)    # DenseSigmoidalFlow is called 
            start = end
...
class DenseSigmoidFlow(BaseFlow):
    ...
    def __init__(self, in_dim, hidden_dim, out_dim):
        ...
        # parameters a_,b_,u_,w_ are subsets of the array dsparams 
        self.act_a = lambda x: nn_.softplus(x)                  # softplus ensures positive values                      
        self.act_b = lambda x: x
        self.act_w = lambda x: nn_.softmax(x,dim=3)             # softmax ensures the summation is 1 in the 4th dimension        
    ...
    def forward(self, x, logdet, dsparams):
        # defining the parameters for dense sigmoidal flow with added regulizers
        inv = np.log(np.exp(1-nn_.delta)-1) 
        ...
        """
        Lines 462-467 perform the calculation for dense sigmoidal flow.
        The code computes the triangular map for all components of the 
        input vector x without the need for explicit loops. This is achieved by
        calculating parameter arrays simultaneously and storing them in an extra dimension.
        ...
        # the calculations below are not necessary for our purpose
        ...
        logj = torchkit.utils.log_sum_exp(logj,3).sum(3)
        # n, d, d2, d1
        ...
        logdet = torchkit.utils.log_sum_exp(
